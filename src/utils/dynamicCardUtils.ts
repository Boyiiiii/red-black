...
export const generateDynamicCard = (
  ...
  let baseWinChance = isColorBet ? 0.55 : 0.30; // Better than casino odds
  ...
  // CASHOUT PROGRESSION: Encourage progress toward cashout milestones
  const cashoutProgress = cashoutWins / 5; // Progress toward 5-win cashout
  ...
  if (bettingStats.favoriteChoice === userChoice && bettingStats.totalBets > 8) {
    const favoriteCount = isColorBet 
      ? bettingStats.colorBets[userChoice as 'red' | 'black']
      : bettingStats.suitBets[userChoice as 'hearts' | 'diamonds' | 'clubs' | 'spades'];
    
    if (favoriteCount > bettingStats.totalBets * 0.5) { // If 50%+ of bets are this choice
  ...
  const finalWinChance = Math.max(0.15, Math.min(0.85, baseWinChance + winChanceModifier));
...
const generateWinningCard = (userChoice: BetChoice): Card => {
  ...
  if (userChoice === 'red') {
    ...
    // Specific suit bet
    const color = userChoice === 'hearts' || userChoice === 'diamonds' ? 'red' : 'black';
    return { suit: userChoice as any, value, color };
  }
};
...
export const updateBettingStats = (
  ...
  if (isColorBet) {
    ...
    newStats.suitBets[betChoice as 'hearts' | 'diamonds' | 'clubs' | 'spades'] += 1;
  }
